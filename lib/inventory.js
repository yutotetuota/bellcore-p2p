...
/**
 * A constructor for inventory related Vertcoin messages such as
...
Inventory.forBlock = function(hash) {
  return Inventory.forItem(Inventory.TYPE.WITNESS_BLOCK, hash);
};
...
Inventory.forFilteredBlock = function(hash) {
  return Inventory.forItem(Inventory.TYPE.FILTERED_WITNESS_BLOCK, hash);
};
...
Inventory.forTransaction = function(hash) {
  return Inventory.forItem(Inventory.TYPE.WITNESS_TX, hash);
};
